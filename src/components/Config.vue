<template>
  <div class="shoe-configurator">
    ...
    <div class="configurator">
      <a class="configurator__arrow" @click="
      ...
      <div class="configurator__flex" v-if="(currentPartIndex && currentPartIndex < 4) || currentPartIndex == 0
          ">
        <div>
        ...
        <div class="configurator__flex1">
          <div class="configurator__flex2">
            <div v-for="color in colorOptions" :key="color" class="configurator__options"
              @click="updateColor(shoePart, color)">
              <div class="configurator__circle" :style="{ backgroundColor: color }"></div>
            </div>
          </div>
          <div v-if="shoePart === 'inside' || shoePart === 'outside'">
            <div class="configurator__flex2">
              <div v-for="material in materialOptions" :key="material" class="configurator__options"
                @click="updateMaterial(materialPart, material)">
                <div class="configurator__circle" :style="{ backgroundImage: `url(${material})` }"></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="configurator__flex" v-if="currentPartIndex === 4">
        ...
        <div class="configurator__flex2">
          <div v-for="jewelType in jewelOptions" :key="jewelType" :id="`${jewelType}jewel`" class="configurator__options"
            @click="updateJewel(jewelType)">
            <div class="configurator__circle" :style="{
      ...
      <div class="configurator__flex" v-if="currentPartIndex === 5">
        ...
        <div class="configurator__initials-container">
          <input class="configurator__checkbox" type="radio" id="inital-yes" :value="false" @change="initialOn" />
          <label for="inital-yes">yes</label>
          <input class="configurator__checkbox" type="radio" id="inital-no" :value="true" @change="initialOff" />
...
export default {
  ...
  mounted() {
    ...
    this.socket = new WebSocket("wss://sneaker-back.onrender.com/primus");
  ...
  // Check if selectedColors and selectedMaterials are defined
  if (!this.selectedColors || !this.selectedMaterials) {
    console.error("selectedColors or selectedMaterials is not defined");
    return;
  }
